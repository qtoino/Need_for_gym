...
def get_velocity_force(velocity):
	if abs(current_force-2500) > 400:
		add_current_force = 200
	else: 
		add_current_force = 0
	return add_current_force * (1/60) * alfa + (1 - beta)*velocity - enemy_velocity
...
def game_intro():
	...
	while gameintro==False:
		...
		if button(600, 400, 70, 30, 'Força', white, bright_green, green,25,606,406):
			time_exe = 5000
			return 2
		if button(300, 400, 135, 30, 'Jogo1 - BONUS', white, bright_blue, blue,25,306,406):
			time_exe = 20000
			enemy_velocity = 0.012
			return 3
...
def gameloop2():
	...
	while not cancel:
		...
		if int(max_force) >= 4095:
			message("Max force reached!",bright_green,50,10,550)
...
def gameloop3():
	...
	while not cancel:
		...
		back()
		velocity = get_velocity_force(velocity)
		y += velocity * clock.get_time()
		y_tree += 10
		if velocity > 0:
			y_tree += 10*velocity * clock.get_time()
...
if __name__ == "__main__":
	...
	if flag == 1:
		...
		p = threading.Thread(target=filter_acceleration)
		p.start()
		o = threading.Thread(target=gameloop3)
		o.start()
	elif flag == 0:
		print('Exit')
		quit()
